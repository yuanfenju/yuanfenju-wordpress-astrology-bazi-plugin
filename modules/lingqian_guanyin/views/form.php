<div class="yfj-form-container" data-module="<?php echo esc_attr($module_id); ?>">
    ...
    <!-- 观音祈福卡片 UI -->
    <div id="yfj-qiuqian-ui" style="text-align: center; padding: 30px 20px 40px 20px; background: linear-gradient(to bottom, #dcfce7, #bbf7d0); border-radius: 12px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 2px solid #fff;">
        ...
        <div style="margin-bottom: 20px; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.15); display: inline-block;">
            <img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($this->t('观音灵签')); ?>" style="width: 100%; max-width: 400px; height: auto; display: block;">
        </div>

        <h3 style="margin: 0 0 15px 0; color: #16a34a; font-size: 26px; font-weight: bold; letter-spacing: 2px;">
            <?php echo $this->t('观音灵签'); ?>
        </h3>

        <p style="font-size: 15px; color: #334155; margin: 0 0 30px 0; line-height: 1.8; font-weight: 500;">
            <?php echo $this->t('大慈大悲，救苦救难。菩萨垂迹，为您解答心中疑惑。'); ?>
        </p>
        ...
        <form class="yfj-ajax-form" onsubmit="yfjStartShake()">
            ...
            <!-- 传给后端的类型为 guanyin -->
            <input type="hidden" name="lingqian_type" value="lingqian_guanyin">
            <?php wp_nonce_field('yfj_nonce', 'yfj_nonce_field'); ?>

            <button type="submit" style="background: #16a34a; color: #fff; border: 2px solid rgba(255,255,255,0.5); padding: 16px 55px; font-size: 18px; border-radius: 50px; cursor: pointer; font-weight: bold; box-shadow: 0 4px 15px rgba(22, 163, 74, 0.3); transition: all 0.2s ease;">
    ...
    <div class="yfj-loading" style="display:none; text-align:center; padding: 40px 20px; background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0;">
        ...
        <div style="color: #16a34a; font-weight: bold; font-size: 16px; letter-spacing: 1px;">
            <?php echo $this->t('正在为您求取观音灵签...'); ?>
        </div>
    </div>
