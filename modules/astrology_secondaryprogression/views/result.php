...
// 第一部分：单盘视角 (次限推运层 Person A)
$progression_data    = $chart_data['person_a_natal'] ?? [];
$progression_planets = $progression_data['planetData'] ?? [];
$progression_houses  = $progression_data['housesData'] ?? [];

// 第二部分：双盘视角 (Synastry)
$synastry          = $chart_data['synastry'] ?? [];
$house_overlays    = $synastry['house_overlay_a_in_b'] ?? []; // 推运星入本命宫
$aspect_list       = $synastry['planet_aspects'] ?? [];       // 推运星与本命星相位
...
<style>
    ...
    /* 分隔横幅 */
    .yfj-section-banner { background-color: #d1ecf1; color: #0c5460; padding: 12px; text-align: center; font-weight: bold; font-size: 15px; border-radius: 6px; margin: 30px 0 20px 0; border: 1px solid #bee5eb; }
    .yfj-section-banner.warning { background-color: #fff3cd; color: #856404; border-color: #ffeeba; }
...
<div class="yfj-sec-prog-result">
    ...
    <div class="yfj-info-header">
        <div class="yfj-info-box" style="border-top: 3px solid #00d0ce;">
            ...
            <p style="margin:4px 0; font-size:13px;"><strong><?php echo $this->t('经度：'); ?></strong><?php echo esc_html($natal_info['longitude'] ?? '-'); ?> &nbsp; <strong><?php echo $this->t('纬度：'); ?></strong><?php echo esc_html($natal_info['latitude'] ?? '-'); ?></p>
        ...
        <div class="yfj-info-box" style="border-top: 3px solid #ff4c00;">
            <h4 style="margin:0 0 10px 0; color: #ff4c00; font-size: 16px;"><span class="dashicons dashicons-clock"></span> <?php echo $this->t('次限推运信息'); ?></h4>
            <p style="margin:4px 0; font-size:13px;"><strong><?php echo $this->t('公历：'); ?></strong><span style="color:#ff4c00; font-weight:bold;"><?php echo esc_html($target_info['birthday'] ?? '-'); ?></span></p>
            <p style="margin:4px 0; font-size:13px;"><strong><?php echo $this->t('推运方式：'); ?></strong><?php echo $this->t('次限法 (一日一年)'); ?></p>
            <p style="margin:4px 0; font-size:13px;"><strong><?php echo $this->t('经度：'); ?></strong><?php echo esc_html($target_info['longitude'] ?? '-'); ?> &nbsp; <strong><?php echo $this->t('纬度：'); ?></strong><?php echo esc_html($target_info['latitude'] ?? '-'); ?></p>
    ...
    <div class="yfj-svg-wrap">
        <h4 style="margin-top:0; color:#1e293b; font-size: 15px;"><span class="dashicons dashicons-art"></span> <?php echo $this->t('次限推运双轮盘 (内圈本命 / 外圈推运)'); ?></h4>
        <div class="yfj-svg-item"><?php echo $clean_svg; ?></div>
    </div>


    <div class="yfj-section-banner">
        <span class="dashicons dashicons-visibility"></span> <?php echo $this->t('第一部分：次限推运盘自身状态 (单盘视角)'); ?>
    </div>

    <div class="yfj-data-section">
        <div class="yfj-table-box">
            <h5 align="center" style="margin: 0 0 15px 0; font-size:15px;"><b><?php echo $this->t('星体落入星座（次限推运盘）'); ?></b></h5>
            <table class="yfj-table">
                ...
                <tbody>
                <?php foreach($progression_planets as $vo): ?>
                    <tr>
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box">
            <h5 align="center" style="margin: 0 0 15px 0; font-size:15px;"><b><?php echo $this->t('各宫位置（次限推运盘）'); ?></b></h5>
            <table class="yfj-table">
                <thead>
                <tr>
                ...
                <tbody>
                <?php foreach($progression_houses as $vo): ?>
                    <tr>
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box">
            <h5 align="center" style="margin: 0 0 15px 0; font-size:15px;"><b><?php echo $this->t('相位列表（推运盘内部互动）'); ?></b></h5>
            <table class="yfj-table">
                <thead>
                <tr>
                ...
                <tbody>
                <?php foreach($progression_planets as $planet):
                    ...
                    foreach(($planet['planet_allow_degree'] ?? []) as $aspect):
                        ...
                        <tr>
                            ...
                            <td><font color="#ff4c00"><b><?php echo esc_html($p2_name); ?></b></font></td>
                            <td>
    ...
    <div class="yfj-section-banner warning">
        <span class="dashicons dashicons-update"></span> <?php echo $this->t('第二部分：推运引发本命的改变 (双盘视角)'); ?>
    </div>

    <div class="yfj-data-section">
        <div class="yfj-table-box">
            <h5 align="center" style="margin: 0 0 15px 0; font-size:15px;"><b><?php echo $this->t('推运星体 落入 本命宫位'); ?></b></h5>
            <table class="yfj-table">
                <thead>
                <tr style="background-color: #fff4f0;">
                    <th style="color:#ff4c00;"><?php echo $this->t('推运星体'); ?></th>
                    <th style="color:#ff4c00;"><?php echo $this->t('推运星座'); ?></th>
                    <th style="color:#00d0ce;"><?php echo $this->t('落入本命宫位'); ?></th>
                    <th style="color:#ff4c00;"><?php echo $this->t('逆行'); ?></th>
                </tr>
                </thead>
                <tbody>
                <?php foreach($house_overlays as $vo): ?>
                    <tr>
                        ...
                        <td><?php echo esc_html($vo['planet']['sign']["sign_{$sf}"] ?? $vo['planet']['sign']['sign_chinese'] ?? ''); ?>(<?php echo esc_html($vo['planet']['sign']['deg'] ?? 0); ?>°<?php echo esc_html($vo['planet']['sign']['min'] ?? 0); ?>′<?php echo esc_html($vo['planet']['sign']['sec'] ?? 0); ?>″)</td>
                        <td>
                            <?php if (!empty($vo['house_id'])): ?>
                                <font color="#00d0ce"><b><?php echo esc_html($vo['house_id']); ?><?php echo $this->t('宫'); ?></b></font> (<?php echo esc_html($vo['relative_dms']['deg'] ?? 0); ?>°<?php echo esc_html($vo['relative_dms']['min'] ?? 0); ?>′<?php echo esc_html($vo['relative_dms']['sec'] ?? 0); ?>″)
                            <?php else: ?>
                                -
                            <?php endif; ?>
                        </td>
                        <td><?php echo !empty($vo['planet']['is_retrograde']) ? '<font color="red">√</font>' : '-'; ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box">
            <h5 align="center" style="margin: 0 0 15px 0; font-size:15px;"><b><?php echo $this->t('相位列表（推运星 vs 本命星）'); ?></b></h5>
            <table class="yfj-table">
                <thead>
                <tr style="background-color: #fff4f0;">
                    <th><?php echo $this->t('推运 (触发方)'); ?></th>
                    <th><?php echo $this->t('相位'); ?></th>
                    <th><?php echo $this->t('本命 (承受方)'); ?></th>
                    <th><?php echo $this->t('方向'); ?></th>
                    <th><?php echo $this->t('容许度'); ?></th>
                </tr>
                </thead>
        ...
        <div style="padding: 15px;">
            <?php foreach (($chart_desc['summary'] ?? []) as $vo): ?>
                <p style="margin-bottom: 10px; font-size: 14px;">
                    【<font color="#ff4c00"><?php echo esc_html($vo['target']); ?></font>】：<?php echo nl2br(esc_html($vo['description'])); ?>
                </p><br>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box" style="padding: 10px 15px; border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold;"><span class="dashicons dashicons-list-view"></span> <?php echo $this->t('推运星体落入本命宫位解析'); ?></h3>
        </div>
        <div style="padding: 15px;">
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box" style="padding: 10px 15px; border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold;"><span class="dashicons dashicons-list-view"></span> <?php echo $this->t('推运星体落入星座解析'); ?></h3>
        </div>
        <div style="padding: 15px;">
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box" style="padding: 10px 15px; border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold;"><span class="dashicons dashicons-list-view"></span> <?php echo $this->t('推运星与本命星相位解析'); ?></h3>
        </div>
        <div style="padding: 15px;">
    ...
    <div class="yfj-data-section">
        <div class="yfj-table-box" style="padding: 10px 15px; border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
            <h3 style="margin: 0; font-size: 15px; font-weight: bold;"><span class="dashicons dashicons-list-view"></span> <?php echo $this->t('四轴交角解析'); ?></h3>
        </div>
        <div style="padding: 15px;">
