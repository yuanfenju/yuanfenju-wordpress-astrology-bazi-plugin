<div class="yfj-form-container" data-module="<?php echo esc_attr($module_id); ?>">
    <div style="text-align: center; margin-bottom: 25px;">
        <h3 style="margin: 0 0 10px 0; color: var(--yfj-text-dark);"><?php echo $this->t('道家阴盘奇门'); ?></h3>
        <p style="font-size: 13px; color: var(--yfj-text-body); margin: 0;">
            <?php echo $this->t('支持时盘与刻盘，神机暗藏，精准推演阴盘奇门局。'); ?>
        </p>
    </div>
    ...
    <form class="yfj-ajax-form">
        ...
        <div style="margin-bottom: 25px; border: 1px solid #cbd5e1; border-radius: 6px; overflow: hidden;">
            <div style="background: #f1f5f9; padding: 10px 15px; font-size: 14px;font-weight: bold; color: #475569; border-bottom: 1px solid #cbd5e1; display: flex; justify-content: space-between; cursor: pointer;" onclick="jQuery('#yfj-yinpan-adv').slideToggle();">
                <span><span class="dashicons dashicons-admin-generic"></span> <?php echo $this->t('阴盘奇门起局设置'); ?></span>
                <span style="font-size: 12px; font-weight: normal;"><?php echo $this->t('点击展开/收起'); ?></span>
            </div>
            <div id="yfj-yinpan-adv" style="padding: 15px; background: #fafafa;">

                <div class="form-group" style="margin-bottom: 15px;">
                    <label><?php echo $this->t('盘类型：'); ?></label>
                    <select name="pan_model" id="yfj_yinpan_model" onchange="yfjToggleKepan(this.value)">
                        <option value="0" selected><?php echo $this->t('时盘 (默认)'); ?></option>
                        <option value="1"><?php echo $this->t('刻盘 (精确到刻)'); ?></option>
                    </select>
                </div>

                <div class="form-group" style="margin-bottom: 15px;" id="yfj-ke-sect-wrap" style="display: none;">
                    <label><?php echo $this->t('刻盘流派：'); ?></label>
                    <select name="ke_sect">
                        <option value="0" selected><?php echo $this->t('从当前时辰第0分钟起刻'); ?></option>
                        <option value="1"><?php echo $this->t('从上个时辰尾第55分钟起刻'); ?></option>
                    </select>
                </div>
        ...
        <button type="submit"><?php echo $this->t('排演阴盘奇门'); ?></button>
    </form>

    <div class="yfj-loading" style="display:none;"><?php echo $this->t('阴盘奇门推演中...'); ?></div>
...
<script>
    (function() {
        ...
        // 阴盘刻盘选项动态展示联动
        window.yfjToggleKepan = function(val) {
            var wrap = document.getElementById('yfj-ke-sect-wrap');
    ...
    .catch(err => {
        ...
        jQuery(document).ready(function($) {
            yfjToggleKepan($('#yfj_yinpan_model').val());
            $('#yfj-yinpan-adv').hide(); // 默认折叠高级设置
        });
    })();
</script>