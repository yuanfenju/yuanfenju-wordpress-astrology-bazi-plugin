...
if (empty($data) || !is_array($data)) {
    echo '<div style="color:red; text-align:center; padding: 20px;">' . $this->t('求签失败，暂无数据。') . '</div>';
    return;
}
...
$zhuge = $base['zhuge'] ?? [];
$content = $zhuge['content'] ?? [];

// 诸葛专属小头像路径
$current_lang = get_option('yfj_language', 'zh-cn');
$lang_suffix = ($current_lang === 'zh-tw') ? 'zh-tw' : 'zh-cn';
$image_url = untrailingslashit(YFJ_PLUGIN_URL) . '/assets/image/lingqian/' . $lang_suffix . '/zhuge.jpg';
?>
...
<style>
    ...
    .yfj-block-title { font-weight: bold; color: #ea580c; display: inline-block; margin-bottom: 8px; border-bottom: 1px dashed #fed7aa; padding-bottom: 6px; font-size: 15px; }
...
<div class="yfj-lq-wrapper">
    ...
    <div class="yfj-panel">
        ...
        <div class="yfj-panel-body">
            ...
            <div style="float: right; margin: 0 0 15px 15px; border-radius: 6px; border: 1px solid #e2e8f0; overflow: hidden; width: 110px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                <img src="<?php echo esc_url($image_url); ?>" style="width: 100%; display: block; border-bottom: 3px solid #f97316;">
            </div>

            <p style="margin: 0 0 10px 0; font-size: 15px;">
                <strong><?php echo $this->t('您抽出诸葛灵签第：'); ?></strong>
            ...
            <p style="margin: 0 0 10px 0; font-size: 15px;">
                ...
                <span style="color: #dc2626; font-weight: bold;">
                    <?php echo esc_html(!empty($zhuge['title']) ? $zhuge['title'] : $this->t('无')); ?>
                </span>
            </p>

            <div style="clear: both;"></div>

            <!-- 注解（原名解释）作为长文本单独显示[cite: 13] -->
            <?php if (!empty($content['解释'])): ?>
                <div style="background: #fff7ed; padding: 18px; border-radius: 6px; border: 1px solid #fed7aa; margin-top: 15px;">
                    <p style="margin: 0;">
                        <strong style="color: #0f172a;"><?php echo $this->t('注解：'); ?></strong>
                        <span style="color: #334155; line-height: 1.7;"><?php echo esc_html($content['解释']); ?></span>
                    </p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <div class="yfj-panel">
        ...
        <div class="yfj-panel-body" style="background: #f8fafc;">
            <div class="yfj-block-grid">
                <?php
                // 诸葛灵签专属的具体预测字段[cite: 13]
                $detail_keys = ['功名', '行人', '婚姻', '官司', '丁口', '生意', '疾病', '出行', '失物', '田畜'];
    ...
    <div style="text-align: center; margin-top: 10px;">
        <button onclick="jQuery('.yfj-result-area').hide(); jQuery('#yfj-qiuqian-ui').fadeIn(); jQuery('.yfj-ajax-form').show();"
                style="background: #fff7ed; color: #ea580c; border: 1px solid #fed7aa; padding: 12px 30px; border-radius: 50px; font-size: 14px; font-weight: bold; cursor: pointer;">
