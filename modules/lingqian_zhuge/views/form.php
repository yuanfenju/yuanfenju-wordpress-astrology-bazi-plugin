<div class="yfj-form-container" data-module="<?php echo esc_attr($module_id); ?>">
    ...
    // 诸葛灵签专属图片路径
    $image_url = YFJ_PLUGIN_URL . 'assets/image/lingqian/' . $lang_suffix . '/zhuge.jpg';
    ?>

    <!-- 诸葛祈福卡片 UI -->
    <div id="yfj-qiuqian-ui" style="text-align: center; padding: 30px 20px 40px 20px; background: linear-gradient(to bottom, #ffedd5, #fed7aa); border-radius: 12px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 2px solid #fff;">
        ...
        <div style="margin-bottom: 20px; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.15); display: inline-block;">
            <img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($this->t('诸葛灵签')); ?>" style="width: 100%; max-width: 400px; height: auto; display: block;">
        </div>

        <h3 style="margin: 0 0 15px 0; color: #ea580c; font-size: 26px; font-weight: bold; letter-spacing: 2px;">
            <?php echo $this->t('诸葛灵签'); ?>
        </h3>

        <p style="font-size: 15px; color: #431407; margin: 0 0 30px 0; line-height: 1.8; font-weight: 500;">
            <?php echo $this->t('神机妙算，决胜千里。进退不决之时，借孔明智慧一用。'); ?>
        </p>
        ...
        <form class="yfj-ajax-form" onsubmit="yfjStartShake()">
            ...
            <!-- 传给后端的类型为 zhuge -->
            <input type="hidden" name="lingqian_type" value="lingqian_zhuge">
            <?php wp_nonce_field('yfj_nonce', 'yfj_nonce_field'); ?>

            <button type="submit" style="background: #ea580c; color: #fff; border: 2px solid rgba(255,255,255,0.5); padding: 16px 55px; font-size: 18px; border-radius: 50px; cursor: pointer; font-weight: bold; box-shadow: 0 4px 15px rgba(194, 65, 12, 0.3); transition: all 0.2s ease;">
    ...
    <div class="yfj-loading" style="display:none; text-align:center; padding: 40px 20px; background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0;">
        ...
        <div style="color: #ea580c; font-weight: bold; font-size: 16px; letter-spacing: 1px;">
            <?php echo $this->t('正在为您求取诸葛灵签...'); ?>
        </div>
    </div>
